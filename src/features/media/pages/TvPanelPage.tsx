...
import { Tv, Plus, Heart } from "lucide-react";
...
function TvPanelPage() {
  const { logs, loading, createLog, updateLog, deleteLog, toggleFavorite } = useMediaLogs("tvshow");
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 border border-dashed border-border rounded-xl text-center gap-4">
          ...
          <p className="text-muted-foreground text-sm">
            {favoritesOnly ? "No favorited shows yet." : "No shows logged yet."}
          </p>
          {!favoritesOnly && (
